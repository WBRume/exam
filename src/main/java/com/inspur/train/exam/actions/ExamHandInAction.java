...
public class ExamHandInAction extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		User s1=(User)session.get("USER_INFO");
		String examIds = (String)session.get("EXAM_ID");
		Exam theExam = examDao.findById(Integer.parseInt(examIds));
		
		List<StudentExamScore> sesList=studentExamScoreDao.findByStudentAndExamAndExamPhase(s1, theExam,"试卷初始化");
		if(sesList.size()>1){
			this.addActionError("(exam_id="+theExam.getId()+")的成绩多于一个。");
			return ERROR;
