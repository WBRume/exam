...
public class JFreeChartTest1 {
	...
	public static void main(String[] args) throws IOException {
		...
		JFreeChart chart = ChartFactory.createBarChart3D("成绩分布图", "姓名", "成绩", dataset, PlotOrientation.VERTICAL, true,
				false, false);
		// String filename = ServletUtilities.saveChartAsPNG(chart, 450, 300,
		// null, session);
		CategoryPlot plot = chart.getCategoryPlot();// 获得图表区域对象
		plot.setRenderer(new CustomRenderer());
		ChartUtilities.saveChartAsPNG(new File("d:\\test1.png"), chart, 1000, 600);
	}
