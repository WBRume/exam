...
<div class="container">
    <form class="col s12" name="form1" method="post" action="examsummary">
        ...
        <div id="positioner">
            <div id="positioner-content">
                <s:if test="choiceList!=null && choiceList.size()>0">选择题:<br>
                    <s:iterator value="choiceList" status="st" var="item">
                        <a id="choiceTag<s:property value='#st.index+1'/>"
                           href="#choice<s:property value='#st.index+1'/>">第<s:property value='#st.index+1'/>题</a>&nbsp;
                    </s:iterator>
                </s:if>
                <s:if test="blankFillingList!=null && blankFillingList.size()>0"><br>
                    <hr>
                    填空题:<br>
                    <s:iterator value="blankFillingList" status="st" var="item">
                        <a id="blankFillingTag<s:property value='#st.index+1'/>"
                           href="#blankFilling<s:property value='#st.index+1'/>">第<s:property value='#st.index+1'/>题</a>&nbsp;
                    </s:iterator>
                </s:if>
                <s:if test="judgeList!=null && judgeList.size()>0"><br>
                    <hr>
                    判断题:<br>
                    <s:iterator value="judgeList" status="st" var="item">
                        <a id="judgeTag<s:property value='#st.index+1'/>"
                           href="#judge<s:property value='#st.index+1'/>">第<s:property value='#st.index+1'/>题</a>&nbsp;
                    </s:iterator>
                </s:if>
                <hr>
            ...
            <a id="togglePositionerBtn" class="brown-text text-darken-4 waves-effect waves-light btn-small"
               onclick="toggleShowPositioner()">关闭题目定位辅助</a>
        </div>
        <s:if test="choiceList!=null && choiceList.size()>0"><h4>选择题</h4></s:if>
        <table class="mytable">

            <s:iterator value="choiceList" status="st" var="item">
                <tr style="border-radius:2px; box-shadow: 5px 5px 3px #aaa;">
                    <td>
                        <table>
                            <tr id="choice<s:property value="#st.index+1"/>" style="height:200px;">
                                <td width="40px" style="vertical-align:middle;" class = "choicetd1">
				...
				<s:property value="#st.index+1"/>.</span>
                                ...
                                <td style="vertical-align:middle;"><s:property
                                        value="@com.inspur.train.exam.actions.ExamDetailShowAction2@showContentWithImage(content)"
                                        escapeHtml="false"/></td>
                                <td style="vertical-align:middle;">
				<span id="choice_answer_q<s:property value="#st.index+1"/>" class="blue-text text-darken-2">
					<s:property
                            value="@com.inspur.train.exam.actions.ExamDetailShowAction2@determineChoiceAnswer(#st.index)"/>
				</span>
                                </td>
                            </tr>
                            <tr style="border-bottom: 1px solid #eee;">
                                <td colspan="3">
                                    <table>
                                        <tr>
                                            <td width="60px" class = "choicetd1">
                                                <input type="<s:if test="@com.inspur.train.exam.actions.ExamDetailShowAction2@isMultipleChoice1(answer)">checkbox</s:if><s:else>radio</s:else>"
                                                       name="choice_q<s:property value="#st.index+1"/>"
                                                       id="q<s:property value="#st.index+1"/>_choiceA"
                                                       class="with-gap" value="A" <s:property
                                                        value="@com.inspur.train.exam.actions.ExamDetailShowAction2@determineChecked(#st.index,'A')"/>
                                                       onclick="changeChoice(<s:property value='#st.index+1'/>,
                                                           <s:property
                                                                   value='@com.inspur.train.exam.actions.ExamDetailShowAction2@isMultipleChoice1(answer)'/>)"/>
                                        ...
                                        <tr>
                                            <td width="60px">
                                                <input type="<s:if test="@com.inspur.train.exam.actions.ExamDetailShowAction2@isMultipleChoice1(answer)">checkbox</s:if><s:else>radio</s:else>"
                                                       ...
                                                       class="with-gap" value="B" <s:property
                                                        value="@com.inspur.train.exam.actions.ExamDetailShowAction2@determineChecked(#st.index,'B')"/>
                                                       onclick="changeChoice(<s:property value="#st.index+1"/>,
                                                           <s:property
                                                                   value='@com.inspur.train.exam.actions.ExamDetailShowAction2@isMultipleChoice1(answer)'/>)"/>
                                        ...
                                        <tr>
                                            <td width="60px">
                                                <input type="<s:if test="@com.inspur.train.exam.actions.ExamDetailShowAction2@isMultipleChoice1(answer)">checkbox</s:if><s:else>radio</s:else>"
                                                       ...
                                                       class="with-gap" value="C"  <s:property
                                                        value="@com.inspur.train.exam.actions.ExamDetailShowAction2@determineChecked(#st.index,'C')"/>
                                                       onclick="changeChoice(<s:property value="#st.index+1"/>,
                                                           <s:property
                                                                   value='@com.inspur.train.exam.actions.ExamDetailShowAction2@isMultipleChoice1(answer)'/>)"/>
                                        ...
                                        <tr>
                                            <td width="60px">
                                                <input type="<s:if test="@com.inspur.train.exam.actions.ExamDetailShowAction2@isMultipleChoice1(answer)">checkbox</s:if><s:else>radio</s:else>"
                                                       ...
                                                       class="with-gap" value="D"  <s:property
                                                        value="@com.inspur.train.exam.actions.ExamDetailShowAction2@determineChecked(#st.index,'D')"/>
                                                       onclick="changeChoice(<s:property value="#st.index+1"/>,
                                                           <s:property
                                                                   value='@com.inspur.train.exam.actions.ExamDetailShowAction2@isMultipleChoice1(answer)'/>)"/>
        ...
        <table class="mytable">
            <s:iterator value="blankFillingList" status="st" var="item">
                <tr id="blankFilling<s:property value='#st.index+1'/>"
                    style="height: 200px;margin: 50px;border-radius:2px; box-shadow: 5px 5px 3px #aaa;">
                    <td style="vertical-align:middle;border-bottom: 1px solid #eee;">
                        <span class="blue-text text-lighten-2" style="display:inline-block;width:30px;"><s:property
                                value="#st.index+1"/>.</span>
                        <span><s:property
                                value="@com.inspur.train.exam.actions.ExamDetailShowAction2@replaceBlank(content,#st.index+1)"
                                escapeHtml="false"/>
				</span>
                    </td>
                </tr>
        ...
        <table class="mytable">
            <s:iterator value="judgeList" status="st" var="item">
                <tr id="judge<s:property value='#st.index+1'/>"
                    style="height:200px;border-bottom: 1px solid #eee;border-radius:2px; box-shadow: 5px 5px 3px #aaa;">
                    <td style="vertical-align:middle;">
                ...
                <input type="hidden" name="judge_q<s:property value="#st.index+1"/>"
                       ...
                       value="<s:property value="@com.inspur.train.exam.actions.ExamDetailShowAction2@determineJudgeChecked(#st.index)"/>"/>
            </s:iterator>
        </table>
        <div class="row">
...
<script type="text/javascript">
    ...
    function setQuestionTags() {
        ...
        if(typeof ($("#choice_answer_q" + choiceIdx).text()) !== "undefined") {
        ...
        if(typeof ($("#blankFilling" + blankFillingIdx + " input").val()) !== "undefined") {
        ...
        if(typeof ($("#judge_q" + judgeIdx).val()) !== "undefined") {
